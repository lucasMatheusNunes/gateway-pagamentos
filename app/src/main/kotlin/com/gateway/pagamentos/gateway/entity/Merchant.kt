...
data class Merchant(
    ...
    @ApiModelProperty(
        notes = "Principals datas of address, in this field must be informed: Number, Street, Neighborhood.",
        example = "728, John Colin, Center",
        required = true
    )
    ...
    @Column(name = "state")
    @ApiModelProperty(
        notes = "Code of State in format ISO 3166-2, more info about ISO 3166-2 in http://twixar.me/VSWn",
        example = "SC",
        required = true
    )
    ...
    @Column(name = "country")
    @ApiModelProperty(
        notes = "Code of Country in format ISO 3166-1 alpha-2, more info about 3166-1 alpha-2 in http://twixar.me/ZXWn",
        example = "BR",
        required = true
    )
    @field:[NotEmpty Size(min = 2, max = 2)]
    ...
    @ApiModelProperty(
        notes = "Bank code: 001 (Banco do Brasil S.A.); 033 (Santander); 104 (Caixa Econômica Federal); 237 (Bradesco); 341 (Itau); This field accept numeric values only",
        example = "033",
        required = true
    )
    @field:[NotEmpty Size(min = 1, max = 3) Pattern(
        regexp = "^([0-9]*)$",
        message = "bankCode field accepts numeric values only"
    )]
    val bankCode: String?,

    @Column(name = "bank_branch_number")
    @ApiModelProperty(
        notes = "Branch number, this field accept numeric values only",
        example = "12345",
        required = true
    )
    @field:[NotEmpty Size(min = 1, max = 5) Pattern(
        regexp = "^([0-9]*)$",
        message = "bankBranchNumber field accepts numeric values only"
    )]
    ...
    @ApiModelProperty(
        notes = "Account number, this field accept numeric values only",
        example = "54321",
        required = true
    )
    @field:[NotEmpty Size(min = 1, max = 13) Pattern(
        regexp = "^([0-9]*)$",
        message = "bankAccountNumber field accepts numeric values only"
    )]
    val bankAccountNumber: String?,

    @Column(name = "bank_account_check_digit")
    @ApiModelProperty(
        notes = "Account check digit, this field accept numeric values only",
        example = "2",
        required = true
    )
    @field:[NotNull Size(min = 1, max = 2) Pattern(
        regexp = "^([a-zA-Z0-9]*)$",
        message = "bankAccountCheckDigit field accepts alphanumeric values only"
    )]
    ...
    @ApiModelProperty(
        notes = "Document, this field depends of value the document_type field, accept numeric values only, ",
        example = "98765432109",
        required = true
    )
    @field:[NotEmpty Size(min = 5, max = 14) Pattern(
        regexp = "^([0-9]*)$",
        message = "document field accepts numeric values only"
    )]
    val document: String?,

    @Column(name = "bank_account_type")
    @ApiModelProperty(
        notes = "Type account: 1 (Checking account); 2 (Savings account); 3 (Checking account joint); 4 (Savings account joint);",
        example = "1",
        required = true
    )
    ...
    @field:[NotEmpty Size(min = 1, max = 7) Pattern(
        regexp = "^([0-9]*)$",
        message = "phoneCountryCode field accepts numeric values only"
    )]
